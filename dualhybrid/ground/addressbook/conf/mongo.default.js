	...
	connectors: {
		mongo: {
			...
			url: process.env.DB_USER ? 
				`mongodb://${process.env.DB_USER}:${process.env.DB_PASSWORD}@${process.env.DB_HOST || 'localhost'}/${process.env.DB_NAME || 'admin'}?authSource=admin` :
				`mongodb://${process.env.DB_HOST || 'localhost'}/${process.env.DB_NAME || 'admin'}?authSource=admin`,
