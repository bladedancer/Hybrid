...
SECRET_NAME=$1
DOMAIN=${2:-bladedancer.asuscomm.com}
...
kubectl create secret generic sda-secrets --namespace apic-control --from-file=publicKey=central/sda_public_key --from-file=privateKey=central/sda_private_key.pem --from-file=password=central/password -o yaml
